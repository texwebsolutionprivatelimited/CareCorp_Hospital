...
const Footer = () => {
  ...
  const socialLinks = [
    { icon: FaFacebookF, href: hospitalInfo.facebookUrl || '#', label: 'Facebook' },
    { icon: FaTwitter, href: hospitalInfo.twitterUrl || '#', label: 'Twitter' },
    { icon: FaInstagram, href: hospitalInfo.instagramUrl || '#', label: 'Instagram' },
    { icon: FaYoutube, href: hospitalInfo.youtubeUrl || '#', label: 'YouTube' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                {socialLinks.map((social) => (
                  <a
                    ...
                    className="w-10 h-10 flex items-center justify-center rounded-full border border-slate-300 text-slate-500 hover:bg-primary hover:border-primary hover:text-white transition-all duration-300"
                  >
              ...
              {/* App Download Badges */}
              {(hospitalInfo.playStoreUrl || hospitalInfo.appStoreUrl) && (
                <div className="mt-8">
                  <h4 className="text-sm font-semibold text-slate-800 mb-3">Download Our App</h4>
                  <div className="flex flex-wrap items-center gap-4">
                    {/* Google Play Badge */}
                    {hospitalInfo.playStoreUrl && (
                      <a
                        href={hospitalInfo.playStoreUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-3 bg-black text-white px-4 py-2 rounded-xl border border-slate-700 hover:border-slate-500 hover:scale-105 transition-all duration-300 w-max"
                      >
                    ...
                    {/* App Store Badge */}
                    {hospitalInfo.appStoreUrl && (
                      <a
                        href={hospitalInfo.appStoreUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-3 bg-black text-white px-4 py-2 rounded-xl border border-slate-700 hover:border-slate-500 hover:scale-105 transition-all duration-300 w-max"
                      >
            ...
            <div>
              <h4 className="text-lg font-semibold font-heading text-slate-800 mb-1">
                Quick Links
              </h4>
              ...
              <ul className="space-y-4">
                ...
                <li>
                  <a
                    ...
                    className="flex items-center gap-3 text-slate-600 hover:text-primary transition-colors duration-200"
                  >
                ...
                <li>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 text-slate-600 hover:text-green-600 transition-colors duration-200"
                  >
                ...
                <li className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    <p className="text-slate-600">
                      <span className="text-slate-800 font-medium">Weekdays:</span>{' '}
                      {hospitalInfo.workingHours.weekdays}
                    </p>
                    <p className="text-slate-600">
                      <span className="text-slate-800 font-medium">Sunday:</span>{' '}
                      {hospitalInfo.workingHours.sunday}
                    </p>
                    <p className="text-slate-600">
                      <span className="text-slate-800 font-medium">Emergency:</span>{' '}
